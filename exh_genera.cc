...
void optimal_billboard_schedule(int k, const MI& Inc, vector<fd>& perm,
    vector<bool>& used, const vector<pair<int, int>>& films_by_rest,
    ...
    if (k == perm.size()) { //all films have been placed
        ...
        for (int u = 0; u < num_films; ++u) { //loop for each film
            if (not used[films_by_rest[u].first]) {
                ...
                optimal_billboard_schedule(k + 1, Inc, perm, used, films_by_rest, current_festival, shortest_festival);
...
int main()
{
    ...
    optimal_billboard_schedule(0, Inc, perm, used, films_by_rest, 0, INT_MAX); //no faria falta passar com a paràmetre alguns atributs
    //print_projection();
}
