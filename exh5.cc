...
{ //passem per copia pq no volem que els canvis es guardin
    for(int day = 1; day < reproduced_together.size(); ++day){
        cout << "used_rooms " << used_rooms[day] << " num_rooms " << num_rooms << endl;
        if(used_rooms[day] < num_rooms and
           not reproduced_together[day][current_film]){
               cout << "day" << day<< endl;
               return day;
           }
    }
        ...
        for (int film = 0; film < num_films; ++film) { //loop for each film
            if (not used[film]) {
                ...
                if(day_of_projection < 0){
                    ++current_festival;
                    cout << "current_festival " << current_festival << endl;
                ...
                cout << "USED_ROOMS : " << endl;
                for(int y : used_rooms) cout << y << ' ';
                cout << endl;
